...
public class ArmorStandProcessor extends StructureEntityProcessor {
    ...
    public StructureTemplate.StructureEntityInfo processEntity(ServerLevelAccessor serverLevelAccessor,
        ...
        if (globalEntityInfo.nbt.getString("id").get().equals("minecraft:armor_stand")) {
            ...
            if (!bootsString.equals("minecraft:air")) {
                ((CompoundTag) newNBT.getListOrEmpty("ArmorItems").getFirst()).putString("id", bootsString);
                ((CompoundTag) newNBT.getListOrEmpty("ArmorItems").getFirst()).putByte("Count", (byte) 1);
                CompoundTag bootsTagNBT = new CompoundTag();
                bootsTagNBT.putInt("Damage", 0);
                ((CompoundTag) newNBT.getListOrEmpty("ArmorItems").getFirst()).put("tag", bootsTagNBT);
            }
